...
- App UI is split by nav item for readability: `src/utils.js` (status/date/draft-storage/ICS helpers), `src/components.js` (shared building blocks used by multiple screens — `Button`, `Badge`, `Field`, `Select`, `TableEmptyState`, sort helpers, `SortableTh`, `WizardProgress`, `ProcessTimeline`, `Pager`, `SearchFilters`, `ApplicationsTable`, `PageHeader`, `Modal`, `FilePreview`), one file per nav item under `src/screens/` (`dashboard.js`, `intake.js` — 臨櫃收件 incl. 申請/廢止, `applications.js` — 申請管理 incl. `ApplicationDetailScreen` and its `Build*Details`/`BuildOperatorName` helpers, `reports.js`, `sanctions.js`, `templates.js`, `holidays.js`, `settings.js` — shared by 字典配置／角色權限管理／帳號管理, `logs.js`), and `src/app.js` (`NAV_ITEMS`, `APP_VERSION`, `FRONTEND_CHANGELOG`, `LoginScreen`, `App`, and the render entry point). A component/screen used by only one nav item lives in that item's file; anything used by more than one stays in `components.js`. Keep this grouping when adding or moving functionality.
